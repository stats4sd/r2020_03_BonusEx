---
title: "Modules 1 -3 : Additional Exercises"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
*3.2 Produce a line plot showing the trend over time of the sum total of plays of all songs combined for each year. Label the axes clearly*
    ...
    group_by(Year) %>%
      summarise(CombinedTotal=sum(TotalPlays)) %>%
        ggplot(aes(y=CombinedTotal,x=Year))+
         geom_line()+
           labs(y="Total Combined Plays of Songs",x="Year of Release",main="Song Plays on Classic Rock Radio by Release Year")
```



*3.3 Find out the most popular "one-hit wonder" in classic rock radio. In other words - I would like to find the song with the highest number of plays, among artists who only had one of their songs played on the classic rock radio stations*
...
#We could use summarise here - but we would also have to remember to summarise the TotalPlays variable as well as we are still going to need this
#But since we are only interested in the groups with one entry, any summary function will do as mean, min, max, median will all be the same, which will be the original value!
#Then filter to just those with 1 song
#Then filter among that subset to the max of total plays
#Sequencing of the commands is crucial here! Flip the final two lines around, or combine them in the same line and you will not reach the correct answer

#American radio clearly need to learn some more Phil Collins songs. As if 'Against All Odds' isnt on their playlist!!?

ClassicRock %>% 
  group_by(Artist) %>%
    summarise(UniqueSongs=n(),TotalPlays=mean(TotalPlays)) %>%
...
*3.4 I am interested to find a list of artists who had extremely short, but very productive careers. Find me a list of artists where all of their songs in the data were released in the same calendar year, and who also have at least three songs with more than 5 plays*
